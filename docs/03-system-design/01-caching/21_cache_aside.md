...
Cache-aside pattern: application checks cache first, loads from source if miss, stores result. Simple but application-side logic required; potential race conditions on multiple misses. Best for read-heavy workloads with acceptable miss latency. Risk: thundering herd if not protected.

**Scale Metrics:**
- Sub-millisecond cache, on-demand loading, 95%+ hit ratio
