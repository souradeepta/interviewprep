...
Cache warming preloads likely-to-be-needed items before serving traffic, reducing cold start penalty. Strategies: replaying historical workloads, precomputing hot items, or priming from previous state. Critical for latency-sensitive services to achieve consistent performance from startup.

**Scale Metrics:**
- 100M+ items, <1 second warm-up, 90%+ hit ratio on startup
