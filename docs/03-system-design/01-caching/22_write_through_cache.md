...
Write-through cache: data written to both cache and source simultaneously, ensuring cache always valid. Simplifies consistency but increases write latency (source latency dominates). Good for systems requiring strong consistency; poor for write-heavy workloads.

**Scale Metrics:**
- Synchronous writes, guaranteed consistency, higher latency
