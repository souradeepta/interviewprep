...
**Key Challenge:** Massive feed generation at global scale with sub-second latency
...
**Data Volume (Daily):**
- Posts: 2B users × 0.5 posts/day = 1B posts/day
...
1000 requests/minute per user, 10K posts/day per user
...
app = FastAPI(title="Facebook Social Network", version="1.0.0")
...
```java
package com.example.facebook_social_network.model;
...
```java
package com.example.facebook_social_network.repository;

import com.example.facebook_social_network.model.Post;
...
```java
package com.example.facebook_social_network.service;

import com.example.facebook_social_network.model.Post;
import com.example.facebook_social_network.repository.PostRepository;
...
```java
package com.example.facebook_social_network.controller;

import com.example.facebook_social_network.model.Post;
import com.example.facebook_social_network.service.PostService;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
...
```java
package com.example.facebook_social_network.config;
...
Facebook Social Network is a mission-critical system serving 3B+ users globally.
...
- Concurrent users: 2B+ DAU, 500K+ RPS peak
...
**Consistency:**
- Model: Eventual consistency for feeds, strong for critical data
...
**Technical Metrics:**
- P50 latency: < 50ms
- P99 latency: P99 < 500ms for feed
- P99.9 latency: < 1s
- Availability: 99.99%
- Error rate: < 0.1%
- Cache hit ratio: > 95%
...
1. System operates at scale: 3B+ users
2. Maintains SLOs: P99 < 500ms for feed latency, 99.99% availability
