#!/usr/bin/env python3
"""Add 30 comprehensive real-world apps concepts (6-35) to 05-real-world-apps."""
import os
output_dir = "docs/system_design/05-real-world-apps"
os.makedirs(output_dir, exist_ok=True)

APPS = [
    "Uber/Lyft Ride Sharing", "Airbnb Booking Platform", "Netflix Streaming",
    "Spotify Music Streaming", "Instagram Social Network", "Twitter X",
    "WhatsApp Messaging", "Slack Workspace Chat", "Zoom Video Conferencing",
    "TikTok Short Videos", "Google Search Engine", "Amazon E-commerce",
    "Facebook Social Network", "Discord Gaming Chat", "GitHub Code Repository",
    "Notion Workspace", "Figma Design Tool", "Stripe Payment Processing",
    "DoorDash Food Delivery", "Twitch Live Streaming", "Pinterest Image Sharing",
    "Snapchat Stories", "Telegram Messenger", "WeChat Super App",
    "LinkedIn Professional Network", "Medium Blogging", "Reddit Community",
    "Quora Q&A", "StackOverflow Developer Q&A", "Dropbox File Sync"
]

CONCEPTS = {
    f"{i:02d}_{'_'.join(app.lower().split())}": {
        "title": f"{app} - System Design",
        "scale": "Millions of users, petabyte storage, global scale",
        "overview": f"Design and architecture of {app} including user management, data flow, scalability, and real-time features."
    }
    for i, app in enumerate(APPS, start=6)
}
...
def generate_concept_file(concept_num, concept_key, concept_data):
    ...
    # Clean key for filename (remove slashes)
    clean_key = concept_key.replace('/', '_').replace('\\', '_')
...
"""
    ...
    filename = f"{output_dir}/{concept_num:02d}_{clean_key}.md"
    with open(filename, 'w') as f:
        f.write(content)
    print(f"✅ Created: {concept_num:02d}_{clean_key}.md")

print("💾 Creating 30 new real-world apps concepts (6-35)...")
print("=" * 70)
for idx, (key, data) in enumerate(CONCEPTS.items(), start=6):
    generate_concept_file(idx, key, data)
print("=" * 70)
print(f"✨ Created 30 comprehensive real-world apps concepts!")
