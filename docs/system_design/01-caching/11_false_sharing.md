...
False sharing occurs when different threads access different variables on same cache line, causing unnecessary coherence traffic. Cache lines (typically 64 bytes) move between cores even though threads access unrelated data, killing multi-threaded performance. Padding and layout optimization essential for high-performance concurrent code.

**Scale Metrics:**
- 100+ threads, 50% performance loss potential, nanosecond overhead
