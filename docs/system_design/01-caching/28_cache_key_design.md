...
Cache keys must be deterministic, collision-free, and version-aware. Strategies: include version in key (foo:v2:user:123), namespace by tenant (tenant:123:foo), or use hash of compound key. Poor key design causes inconsistent data; good design enables versioning and namespacing.

**Scale Metrics:**
- 1B+ keys, collision avoidance, versioning support
