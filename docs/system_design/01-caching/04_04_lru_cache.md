...
LRU cache evicts the least recently used item when capacity is exceeded, based on temporal locality principle. Used in CPU caches, page replacement, and application-level caching. Requires O(1) lookup and O(1) eviction using doubly-linked lists with hashmap, tracking access patterns efficiently.

**Scale Metrics:**
- 100M+ cache entries, sub-microsecond eviction, 95%+ hit ratio
