# Cache Bypass and Direct Backend Access

## System Overview

Some queries bypass cache for guaranteed freshness (consistency-critical reads) or due to being non-cacheable. Bypass policies balance consistency needs with latency/cost. Examples: freshness-critical queries, one-time large requests, or invalidation uncertainty.

**Scale Metrics:**
- Selective caching, reduce cache pollution, adaptable consistency
