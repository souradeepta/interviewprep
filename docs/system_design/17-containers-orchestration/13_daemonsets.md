# DaemonSets and Node-Level Services

## System Overview

DaemonSets ensure a specific pod runs on every node in a cluster, making them ideal for system-level services like monitoring agents, log collectors, and network plugins. They automatically handle node additions/removals and respect node affinity/taints, enabling infrastructure teams to deploy node-critical workloads consistently across heterogeneous clusters.

**Scale Metrics:**
- 1000+ nodes, deploy to every node simultaneously
