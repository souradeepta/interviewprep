...
Kubernetes Services provide stable network endpoints for accessing pods, abstracting the ephemeral pod IPs. Service discovery (via DNS or environment variables) enables loose coupling between services. Multiple service types (ClusterIP for internal, NodePort for external, LoadBalancer for cloud) provide flexibility in exposure patterns.

**Scale Metrics:**
- 10K+ services, 1M+ QPS load balancing, <10ms discovery
