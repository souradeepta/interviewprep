...
Deployment strategies (Rolling, Blue-Green, Canary) control how new versions are rolled out to minimize downtime and blast radius of bad deployments. Rolling updates gradually replace old pods with new ones, Blue-Green switches entire traffic at once, and Canary routes a percentage of traffic to validate new versions. Kubernetes Deployments natively support rolling updates with configurable surge/unavailability.

**Scale Metrics:**
- 1000+ pod replicas, zero-downtime deployments, <30s update
