...
**Key Challenge:** Timeline generation with trending hashtags and real-time updates
...
**Data Volume (Daily):**
- Tweets: 350M DAU × 0.3 tweets = 105M tweets/day
...
app = FastAPI(title="Twitter Feed", version="1.0.0")
...
```java
package com.example.twitter_feed.model;
...
```java
package com.example.twitter_feed.repository;

import com.example.twitter_feed.model.Post;
...
```java
package com.example.twitter_feed.service;

import com.example.twitter_feed.model.Post;
import com.example.twitter_feed.repository.PostRepository;
...
```java
package com.example.twitter_feed.controller;

import com.example.twitter_feed.model.Post;
import com.example.twitter_feed.service.PostService;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
...
```java
package com.example.twitter_feed.config;
...
Twitter Feed is a mission-critical system serving 550M+ users globally.
...
- Concurrent users: 350M+ DAU, 300K+ RPS peak
...
**Technical Metrics:**
- P50 latency: < 50ms
- P99 latency: P99 < 300ms for timeline
- P99.9 latency: < 1s
- Availability: 99.99%
- Error rate: < 0.1%
- Cache hit ratio: > 95%
...
1. System operates at scale: 550M+ users
2. Maintains SLOs: P99 < 300ms for timeline latency, 99.99% availability
